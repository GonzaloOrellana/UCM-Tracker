      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto pointer-events-auto">
          ...
          >
            ...
            <div className="flex items-start justify-between border-b border-white/15 pb-3 mb-4 shrink-0">
              <div>
                ...
                <p className="text-xs text-zinc-300 mt-0.5 font-normal">
                  ...
                  {isLoggedIn && (
                    <span className="text-[11px] text-emerald-400/80 font-normal inline-flex items-center gap-1 ml-2">
                      <ShieldCheck className="w-3 h-3 text-emerald-400/80" /> Se sincronizará en todos tus dispositivos
                    </span>
                  )}
                </p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-8 pr-1 select-none">
              {categories.map((category) => {
                const categoryAvatars = AVAILABLE_AVATARS.filter((a) => a.category === category);
                return (
                  <div key={category} className="space-y-2.5 w-full min-w-0">
                    {/* Category Title (Clean Minimalist Label) */}
                    <h3 className="text-[11px] font-semibold tracking-widest text-white/60 uppercase">
                      {category}
                    </h3>
                      ...
                      {categoryAvatars.map((avatar) => {
                        ...
                        return (
                          <button
                            ...
                            title={avatar.name}
                            className={`relative group shrink-0 w-14 h-14 sm:w-16 sm:h-16 aspect-square rounded-full overflow-hidden bg-[#141625] cursor-pointer transition-all duration-200 focus:outline-none ${
                              isSelected
                                ? 'ring-1 ring-red-500/90 ring-offset-2 ring-offset-[#1e2238] scale-[1.03] shadow-md z-10'
                          ...
                          >
                            <img
                              src={avatar.url}
                              alt={avatar.name}
                              className="w-full h-full object-cover object-center transition-transform duration-300 group-hover:scale-110 rounded-full pointer-events-none"
                              loading="lazy"
                            />
